cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine matcov(cov,x,theta,num)
c
c     This computes the Matern correlation based on 
c     locations x and parameter theta = (range, smoothness)
c
c     The answer is passed back in the array cov.
c     theta is an vector of length 2.
c     cov is an array of num x num.
...
c
      ...
      double precision theta(2)
      double precision bk(50)
      double precision x(num,2)
      parameter( half = 0.5d0, two = 2.0d0 )
      parameter( zero = 0.0d0, one = 1.0d0 )
      parameter( three = 3.0d0 )
c
      dis(i,j) = dsqrt( (x(i,1)-x(j,1))*(x(i,1)-x(j,1))
     +                + (x(i,2)-x(j,2))*(x(i,2)-x(j,2)) )
c
c     t0 = theta(3)
      t1 = one
      t3 = theta(2)
      t2 = two*dsqrt(t3)/theta(1)
c
c     calculate v given theta
c
      nk = 0
      do 21 j = 1, num
       do 11 i = 1, j
